...
export const projectInputSchema = z
  .object({
    ...
    subtitle: z
      ...
      .trim()
      .max(
        FIELD_LIMITS.name.long,
        `Subtitle must be ${FIELD_LIMITS.name.long} characters or fewer`,
      ),
      ...
      .array(
        z
          ...
          .max(
            ...
            `Categories must be ${FIELD_LIMITS.tag.name} characters or fewer`,
          ),
      )
      .max(
        FIELD_LIMITS.tag.maxCount,
        `Up to ${FIELD_LIMITS.tag.maxCount} categories allowed`,
      ),
